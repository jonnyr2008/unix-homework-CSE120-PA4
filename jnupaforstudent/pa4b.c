 ...
 * return the id of the thread that yielded to the thread being given control,
 * i.e., the one returning from its call to MyYieldThread.  More on this below.
 *
 ...
 * Hence it will be creating a new thread, thread 1 (thread id's range
 ...
 * with only two threads whose id's must be 0 and 1.  So when Thread 0
